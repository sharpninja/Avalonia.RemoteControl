...
The client must reject unknown marker protocols before creating an ADB forward. The current client supports `grpc` and `arc-protobuf-v1` for authenticated bridge operations.
...
- `arc-protobuf-v1` uses the bridge client adapter behind the same desktop session/probe workflow for capabilities, snapshots, tree streaming, frame streaming, logs, click, focus, input, and property mutation.
...
Bridge streaming keeps the TCP connection open for long-lived `WatchTree`, `WatchFrames`, and `WatchLogs` responses and ends the stream when the client cancels or closes the socket. The Android app-side listener starts as a loopback-only TCP listener, dispatches authenticated requests through the runtime bridge handler, publishes package-private marker metadata, and stops with the debuggee lifecycle. The Android probe sample writes a per-process debug token into package-private marker JSON, so normal client discovery still uses `adb shell run-as` and does not log the token.
