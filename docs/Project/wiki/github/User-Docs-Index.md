...
- [Getting Started](Getting-Started) - install the tool, add the server package, and connect to a local app.
- [Server Integration](Server-Integration) - configure an Avalonia app as a debuggee.
- [Settings Guide](Settings-Guide) - understand every server, client, ADB, live-view, logging, and security setting.
- [Client Tool](Client-Tool) - launch the desktop client and use the CLI workflows.
- [Android ADB](Android-ADB) - connect to an Android emulator or connected device through `adb`.
- [Codex MCP](Codex-MCP) - launch Codex inside the tool and use the running tool's MCP server.
- [Security](Security) - understand the default safety model before enabling remote control.
- [Troubleshooting](Troubleshooting) - diagnose common startup, connection, ADB, and package issues.
