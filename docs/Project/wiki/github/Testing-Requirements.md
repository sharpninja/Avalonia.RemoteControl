...
| TEST-ADB-009 | Unit tests verify the arc-protobuf-v1 bridge envelope encodes and decodes length-prefixed protobuf request and response frames and rejects oversized frames. |
...
| TEST-ADB-013 | Tests must verify bridge streaming for tree and frame streams, bridge cancellation by socket close, and unsupported capability handling. |
| TEST-ADB-014 | Unit tests verify package-marker adb connect fails before forwarding when pidof shows the package is stopped, and verify an Android bridge connection that closes before a response frame is reported as a clean InvalidOperationException. |
...
| TEST-AVA-005 | Headless Avalonia tests must verify dispatcher-safe frame capture, max size rejection, and frame stream cancellation. |
| TEST-AVA-006 | Headless Avalonia tests must verify pointer, wheel, keyboard, and text input dispatch to the remote root or focused control. |
| TEST-AVA-007 | Validation must cover that frame capture, tree snapshots, and input dispatch use the TopLevel surface when the registered root provider returns a child view. |
...
| TEST-GRPC-008 | Tests must verify authenticated gRPC frame streaming, cancellation, and policy rejection. |
| TEST-GRPC-009 | Tests must verify SendInput policy rejection, enabled dispatch, pointer drag state, keyboard/text routing, and sanitized audit logs. |

## TEST-LIVE-VIEW

| ID | Requirement |
| --- | --- |
| TEST-LIVE-VIEW-008 | Unit tests verify live-view hit testing chooses the deepest visible node whose absolute bounds contain the clicked root-relative point and ignores invisible or out-of-bounds nodes. |

## TEST-LOG

| ID | Requirement |
| --- | --- |
| TEST-LOG-001 | Unit tests verify Debug ILogger messages for incoming client operations and outgoing runtime responses or stream updates, including the non-recursive WatchLogs lifecycle behavior. |
| TEST-LOG-002 | Unit tests verify the Android bridge TCP transport writes Debug diagnostics for bridge request and response frame lifecycle and does not emit per-entry response-frame logs for WatchLogs streams. |
...
| TEST-LOG-005 | Unit tests verify the pop-out log window model shares the same live log rows as the main log panel without starting an additional log stream. |
| TEST-LOG-006 | Unit or UI-model tests verify that popping out logs removes the embedded main-window log list, the pop-out exposes a dock command, and docking restores the embedded list without creating a duplicate log stream. |
| TEST-LOG-007 | Unit tests verify the shared log view model exposes supported verbosity options, tracks selected verbosity changes, and notifies the client UI so embedded and pop-out selectors stay synchronized. |
...
| TEST-PACK-005 | dotnet pack --no-build produces the Avalonia.RemoteControl.Runtime NuGet package. |
