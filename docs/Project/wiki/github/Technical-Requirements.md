...
## TR-DOCKING-LAYOUT-001

**An MVVM Dock factory builds the panel layout and resolves views by dockable id.** — RemoteControlDockFactory subclasses Dock.Model.Mvvm.Factory to build the RootDock/ProportionalDock/ToolDock/DocumentDock tree with legacy proportions; RemoteControlDockViewLocator maps each built-in dockable by id to its panel with the shell view-model as DataContext.
Scope: layer-1+
**Acceptance Criteria:**
- [ ] CreateLayout produces West control-tree, center workspace over logs, and east remote-tools over live-view with matching proportions.
- [ ] Each dockable resolves to its existing panel UserControl bound to the shell view-model.

## TR-DOCKING-PERSISTENCE-001

**The dock layout persists per project via the Dock System.Text.Json serializer.** — RemoteControlDockLayoutStore saves/loads {projectId}.arclayout.json via Dock.Serializer.SystemTextJson, writing atomically and detaching Owner back-references; panel view-models re-attach from the live shell via ContextLocator in InitLayout on load.
Scope: layer-1+
**Acceptance Criteria:**
- [ ] A saved layout round-trips (structure preserved, view-models re-attached) and restores on next launch.
- [ ] A failed save never corrupts the last good layout file.
